...
export function DetectionStreamLog({ streamLogs, isVideoFullscreen }: { streamLogs: StreamLog[], isVideoFullscreen: boolean }) {
  return (
    <div className={`bg-surface-container-lowest p-6 rounded-xl border border-outline-variant/15 flex flex-col ${isVideoFullscreen ? 'h-64 flex-1 min-w-[300px]' : 'flex-1 min-h-[300px] w-full'}`}>
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-on-surface-variant text-sm font-medium">Detection Stream Log</h3>
        <span className="text-[10px] font-bold text-primary px-2 py-0.5 bg-primary-fixed rounded-full">LIVE</span>
      </div>
                ...
                <td className="p-2">
                  <span
                    className={`px-2 py-0.5 rounded-full font-bold ${
                      entry.level === "EMERGENCY" ? "bg-rose-600 text-white animate-pulse" :
                      entry.level === "WARN" ? "bg-error-container text-error" : "bg-slate-200 text-slate-600"
                    }`}
                  >
                    {entry.level}
                  </span>
